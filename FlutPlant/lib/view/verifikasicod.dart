...
import 'package:mysqlshop/modal/codmodel.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:http/http.dart' as http;

class Verifikasicod extends StatefulWidget {
  @override
  _VerifikasicodState createState() => _VerifikasicodState();
}

class _VerifikasicodState extends State<Verifikasicod> {
  ...
  submit() async {
    try {
      var uri = Uri.parse(BaseUrl.productVerifycod);
  ...
  _deletebukti(String id) async {
    final response = await http.post(BaseUrl.deletecod, body: {"idUsers": id});
    final data = jsonDecode(response.body);
    int value = data['value'];
    String pesan = data['message'];
    if (value == 1) {
  ...
  Future<void> _lihatData() async {
    ...
    list.clear();
    final response = await http.get(BaseUrl.lihatcod + idUsers);
    final data = jsonDecode(response.body);
    data.forEach((api) {
      final ab = new CodModel(
        api['id'],
        api['createdDate'],
        api['status'],
        api['idUsers'],
